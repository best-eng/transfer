...
def kb():
    return ReplyKeyboardMarkup([
        [KeyboardButton("📦 Посылка",            web_app=WebAppInfo(url=WEBAPP_URL+"/parcel.html"))],
        [KeyboardButton("🔍 Найти поездку",       web_app=WebAppInfo(url=WEBAPP_URL+"/trip.html"))],
        [KeyboardButton("🚗 Заказать трансфер",   web_app=WebAppInfo(url=WEBAPP_URL+"/rent.html"))],
...
async def webapp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if t=="parcel":
        ...
        lines = ["🆕 <b>НОВАЯ ЗАЯВКА</b>","📌 <b>Тип:</b> Поездка","",
                 "🛣 <b>Маршрут:</b> "+d.get("route",""),
                 "📅 <b>Дата:</b> "+d.get("date","")+" "+d.get("time",""),
        ...
        lines = ["🆕 <b>НОВАЯ ЗАЯВКА</b>","📌 <b>Тип:</b> Аренда","",
